...
async def register_bot_command_menu():
    """
    Registers Telegram's native command menu (the list that pops up when
    you type '/' in the chat) so every command is visible with a short
    description, instead of only working if you already know it.
    """
    from telethon.tl.functions.bots import SetBotCommandsRequest
    from telethon.tl.types import BotCommand, BotCommandScopeDefault

    commands = [
        BotCommand("start", "Greeting + how to use this bot"),
        BotCommand("add", "Add study material (guided, supports multiple files)"),
        BotCommand("done", "Finish an in-progress /add session"),
        BotCommand("delete", "Delete a document (ID) or a user (email/ID)"),
        BotCommand("list", "List all uploaded documents"),
        BotCommand("categories", "Show categories with pricing + type"),
        BotCommand("stats", "Usage statistics"),
        BotCommand("dashboard", "Full user activity list (downloads, opens, searches)"),
        BotCommand("unlock", "Grant monthly category access: email cat_no [days]"),
        BotCommand("unlockfull", "Grant full course access: email cat_no"),
        BotCommand("unlockfile", "Grant single file access: email doc_id"),
        BotCommand("setfullprice", "Set a category's full-course price"),
        BotCommand("setmonthlyprice", "Set a category's monthly price"),
        BotCommand("setoffline", "Mark a category offline + set its location"),
        BotCommand("setonline", "Mark a category back online"),
        BotCommand("setfileprice", "Mark a file premium with its own price"),
        BotCommand("removepremium", "Remove premium status from a file"),
        BotCommand("ban", "Block a user: email"),
        BotCommand("unban", "Unblock a user: email"),
        BotCommand("broadcast", "Set/clear the website's top banner"),
    ]
    try:
        await bot(SetBotCommandsRequest(scope=BotCommandScopeDefault(), lang_code="en", commands=commands))
        print(f"[INFO] Registered {len(commands)} commands in Telegram's command menu.")
    except Exception as e:
        print(f"[WARN] Could not register bot command menu: {e}")


async def main():
    await start_bot_client()
    await register_bot_command_menu()
