...
// ---------------------------------------------------------------
// Online / Offline month-by-month plan selector (homepage section)
// ---------------------------------------------------------------
let onlinePlanFee = 500;
let offlinePlanFee = 500;
let onlineMonths = 1;
let offlineMonths = 1;

function updatePlanPrice(kind, months) {
  const fee = kind === "online" ? onlinePlanFee : offlinePlanFee;
  const valueEl = document.getElementById(`${kind}-months-value`);
  const priceEl = document.getElementById(`${kind}-plan-price`);
  if (valueEl) valueEl.textContent = months;
  if (priceEl) priceEl.textContent = `₹${fee * months}`;
}

function openPlanWhatsApp(kind, months) {
  const fee = kind === "online" ? onlinePlanFee : offlinePlanFee;
  const amount = fee * months;
  const modeLabel = kind === "online" ? "Online" : "Offline";
  const message =
    `Hello, I would like to subscribe to the ${modeLabel} coaching plan.\n\n` +
    `Duration: ${months} month(s)\n` +
    `Amount: Rs. ${amount}\n\n` +
    `Please share payment details / confirm my subscription.`;
  window.open(`https://wa.me/${ownerWhatsappNumber}?text=${encodeURIComponent(message)}`, "_blank");
}

document.getElementById("online-months-minus").addEventListener("click", () => {
  onlineMonths = Math.max(1, onlineMonths - 1);
  updatePlanPrice("online", onlineMonths);
});
document.getElementById("online-months-plus").addEventListener("click", () => {
  onlineMonths = Math.min(12, onlineMonths + 1);
  updatePlanPrice("online", onlineMonths);
});
document.getElementById("online-plan-continue").addEventListener("click", () => openPlanWhatsApp("online", onlineMonths));

document.getElementById("offline-months-minus").addEventListener("click", () => {
  offlineMonths = Math.max(1, offlineMonths - 1);
  updatePlanPrice("offline", offlineMonths);
});
document.getElementById("offline-months-plus").addEventListener("click", () => {
  offlineMonths = Math.min(12, offlineMonths + 1);
  updatePlanPrice("offline", offlineMonths);
});
document.getElementById("offline-plan-continue").addEventListener("click", () => openPlanWhatsApp("offline", offlineMonths));
...
async function loadBranding() {
  try {
    ...
    onlinePlanFee = b.online_plan_fee_inr || onlinePlanFee;
    offlinePlanFee = b.offline_plan_fee_inr || offlinePlanFee;
    if (b.offline_location) {
      document.getElementById("offline-location-text").textContent = `📍 ${b.offline_location}`;
    }
    updatePlanPrice("online", onlineMonths);
    updatePlanPrice("offline", offlineMonths);
  } catch (e) {
